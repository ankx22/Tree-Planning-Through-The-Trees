...
#     sim_stop_time = 60
#     filepath = "./src/sample_maps/map2.txt"
...
#             loc = (np.dot((x_coeffs[4*ind:4*ind+4]).T, t_vec), np.dot(
#                 (y_coeffs[4*ind:4*ind+4]).T, t_vec), np.dot((z_coeffs[4*ind:4*ind+4]).T, t_vec))

#             envi.add_sphere(loc, orange_color)
...
#     print("Shape is: ", pos_x_arr.shape, pos_y_arr.shape, pos_z_arr.shape, vel_x_arr.shape,
#           vel_y_arr.shape, vel_z_arr.shape, acc_x_arr.shape, acc_y_arr.shape, acc_z_arr.shape)
#     final_traj = np.vstack((pos_x_arr, pos_y_arr, pos_z_arr, vel_x_arr,
#                            vel_y_arr, vel_z_arr, acc_x_arr, acc_y_arr, acc_z_arr))
...
#             xyz_bl_des, vel_bl_des, acc_bl_des, yaw_bl_setpoint = user_sm.step(
#                 current_time, xyz_blender, vxyz_blender)

#             yaw_ned = -yaw_bl_setpoint
#             WP_ned = np.array(
#                 [xyz_bl_des[0], -xyz_bl_des[1], -xyz_bl_des[2], yaw_ned])
...
#    goal = [9.8,29.5,3]
    ...
    print(path_array)
    envi.visualize_nodes(path)
    traj = traj_gen.traj_gen(None,path_array)
    print("Total dist", traj.timeinterval_calc())
    ...
    while ind < len(x_coeffs)/4:
        ...
        for t in np.arange(prev_t, curr_t, stp):
            ...
            loc = (np.dot((x_coeffs[4*ind:4*ind+4]).T,t_vec),np.dot((y_coeffs[4*ind:4*ind+4]).T,t_vec),np.dot((z_coeffs[4*ind:4*ind+4]).T,t_vec))
            envi.add_sphere(loc,orange_color)
    ...
    # INIT STATES
    current_time = 0.
    xyz = np.array([0.3,3,-2])#[0.0, 0.0, -0.2]
    vxyz = np.array([0.0, 0.0, 0.0])
    quat = np.array([1.0, .0, .0, .0])
    pqr = np.array([0.0, .0, .0])
    current_ned_state = np.concatenate((xyz, vxyz, quat, pqr))    
